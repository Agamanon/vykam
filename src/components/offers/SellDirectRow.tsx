...
export default function SellDirectRow({ oferta, usuarioId, onEjecutar }: Props) {
  ...
  async function handleVender() {
    if (!usuarioId) { alert('Debes iniciar sesión para realizar una venta'); return }
  ...
  return (
    <tr className="fila-vender-directo">
      ...
      <td>
        <input type="number" className="vender-directo-cantidad" placeholder="0" min="1" max={oferta.cantidad} value={cantidad} disabled={!usuarioId}
          onChange={e => {
